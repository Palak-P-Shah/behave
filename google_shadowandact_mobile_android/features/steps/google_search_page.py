...
def navigate_to_google_page_on_mobile(driver):
    ...
    print(driver.title)
    try:
        frame = driver.switch_to_active_element()
        ide = driver.find_element(By.ID, "introAgreeButton")
        ide.click()
    except NoSuchElementException:
        print("No pop up found on google page launch")


def search_keyword(driver, website_name):
    print("function called search_keyword")
    search_text_box = driver.find_element(By.XPATH, "//input[@type='search']")
...
def launch_app(driver):
    ...
    result = driver.find_element(By.XPATH, "//span[normalize-space()='https://shadowandact.com']")
    time.sleep(2)
    driver.get("https://shadowandact.com")
    # result.click()
    time.sleep(5)
...
def post_page_load_pop_up(driver):
    try:
        event_promo_pop_up = driver.find_element_by_xpath(
          "(//button[@type='button'][normalize-space()='×'])[1]")
